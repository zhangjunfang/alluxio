...
public class HdfsAndAlluxioUtils {
	 ...
	 *            是否删除本地文件
	 */
	public static void copyOrMoveFromLocalFile(FileSystemInfo fileSystemInfo, String src, String dist,
			boolean overwrite, boolean delSrc) {
		FileSystem fs = getFileSystem(fileSystemInfo);
		// local
		...
		try {
			...
			if (localFile.isDirectory()) {
				...
				for (File f : listFiles) {
					copyOrMoveFromLocalFile(fileSystemInfo, f.getAbsolutePath(), convertToPath(dist), overwrite,
							delSrc);
				}
			} else {
				// file
	...
	/**
	 * 此方法用于创建文件快照 需要开启快照支持hdfs dfsadmin -allowSnapshot /path/to/snapshot
	 *
	 * @param fileSystemInfo
	 *            文件系统信息
	 * @param path
	 ...
	 *            新快照名称
	 */
	public static void renameSnapshot(FileSystemInfo fileSystemInfo, String path, String snapshotOldName,
	 ...
	 *            文件输出后是否关闭输出流
	 */
	public static void open(FileSystemInfo fileSystemInfo, String path, OutputStream outputStream, int bufferSize,
			boolean close) {
		FileSystem fs = getFileSystem(fileSystemInfo);
		Path uri = new Path(path);
		try {
